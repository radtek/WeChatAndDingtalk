...
{
    internal class WeChatEnterpriseMessage : IMessage
    {
        #region 单例

        protected static readonly IMessage _instance = new WeChatEnterpriseMessage();
        ...
        private WeChatEnterpriseMessage()
        {
        }
        ...
        public MessageSendResult Send(AppAccountInfo appAccount, MessageInfo message)
        {
            ...
            AppConnectLogHelper.DebugFormat("微信企业号调用微信api发送消息:Token{0};ToOpenId{1}; AgentId{2} ContentUrl{3}, ContentTitle{4}, description:{5}, pic:{6}", token, message.ToOpenId, appAccount.AgentId, message.Content.Url, message.Content.Title, description.ToString(), picUrl);
            ...
            var sendResult = WeChatSDK.QY.Message.SendNews(token, message.ToOpenId, Convert.ToInt32(appAccount.AgentId), message.Content.Title, description.ToString(), message.Content.Url, picUrl);

            if (sendResult.ErrCode == 0)
            {
                result.State = MessageState.Success;
                AppConnectLogHelper.DebugFormat("微信企业号发送消息成功{0}", JsonConvert.SerializeObject(sendResult));
            }
            else
            {
                result.State = MessageState.Failed;
                AppConnectLogHelper.ErrorFormat("微信企业号发送消息失败,errorCode:{0},errorMsg:{1}", sendResult.ErrCode, sendResult.ErrMsg);
            }
