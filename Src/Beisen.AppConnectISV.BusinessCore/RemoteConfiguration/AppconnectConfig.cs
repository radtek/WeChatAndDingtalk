...
namespace Beisen.AppConnectISV.BusinessCore
{
    ...
    public class AppconnectConfig : BaseConfig<AppconnectConfig>
    {
        ...
        public static T GetAppconnectConfigInfo<T>(string nodeType, string key)
        {
            ...
            {
                LogHelper.Instance.Dump(string.Format("为获取到远程配置信息AppconnectConfig,NodeType{0}", nodeType), LogType.Error);
                return default(T);
            }
            if (!appconnectConfigInfo[nodeType].ContainsKey(key))
            {
                LogHelper.Instance.Dump(string.Format("为获取到远程配置信息AppconnectConfig,NodeType{0}, Key:{1}", nodeType, key), LogType.Error);
                return default(T);
            }
